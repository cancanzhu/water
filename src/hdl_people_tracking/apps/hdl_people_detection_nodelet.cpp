...
public:
  ...
  void onInit() override {
    ...
    if(private_nh.param<bool>("static_sensor", false)) {
      ...
      static_sensor = false;
      dynamic_points_sub = mt_nh.subscribe("/velodyne_points", 32, &HdlPeopleDetectionNodelet::callback, this);
      // odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(mt_nh, "/water_uavcan_master/odom", 20));
...
private:
  ...
  void globalmap_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    pcl::fromROSMsg(*points_msg, *cloud);
    globalmap = cloud;
    // attention，globalmap中 仅header.frame_id被后续用到了！
    if (!static_sensor){
      globalmap->header.frame_id = private_nh.param<std::string>("globalmap_frame", "/odom");
    }
...
private:
  ...
  void publish_msgs(const ros::Time& stamp, const pcl::PointCloud<pcl::PointXYZI>::Ptr& filtered, const std::vector<Cluster::Ptr>& clusters) const {
    if(clusters_pub.getNumSubscribers()) {
      ...
      // transform from /velodyne to globalmap->header.frame_id
      tf_listener.waitForTransform(globalmap->header.frame_id, "/velodyne", ros::Time(0), ros::Duration(0.5));
      geometry_msgs::PoseStamped origin;
      geometry_msgs::PoseStamped ans;
      ...
      for(int i=0; i<clusters.size(); i++) {
        ...
        origin.pose.position.x = clusters[i]->min_pt.x();
        origin.pose.position.y = clusters[i]->min_pt.y();
        origin.pose.position.z = clusters[i]->min_pt.z();
        origin.header.stamp = ros::Time();
        tf_listener.transformPose(globalmap->header.frame_id, origin, ans);
        cluster_msg.min_pt.x = ans.pose.position.x;
        cluster_msg.min_pt.y = ans.pose.position.y;
        cluster_msg.min_pt.z = ans.pose.position.z;

        origin.pose.position.x = clusters[i]->max_pt.x();
        origin.pose.position.y = clusters[i]->max_pt.y();
        origin.pose.position.z = clusters[i]->max_pt.z();
        origin.header.stamp = ros::Time();
        tf_listener.transformPose(globalmap->header.frame_id, origin, ans);
        cluster_msg.max_pt.x = ans.pose.position.x;
        cluster_msg.max_pt.y = ans.pose.position.y;
        cluster_msg.max_pt.z = ans.pose.position.z;

        origin.pose.position.x = clusters[i]->size.x();
        origin.pose.position.y = clusters[i]->size.y();
        origin.pose.position.z = clusters[i]->size.z();
        origin.header.stamp = ros::Time();
        tf_listener.transformPose(globalmap->header.frame_id, origin, ans);
        cluster_msg.size.x = ans.pose.position.x;
        cluster_msg.size.y = ans.pose.position.y;
        cluster_msg.size.z = ans.pose.position.z;

        origin.pose.position.x = clusters[i]->centroid.x();
        origin.pose.position.y = clusters[i]->centroid.y();
        origin.pose.position.z = clusters[i]->centroid.z();
        origin.header.stamp = ros::Time();
        tf_listener.transformPose(globalmap->header.frame_id, origin, ans);
        cluster_msg.centroid.x = ans.pose.position.x;
        cluster_msg.centroid.y = ans.pose.position.y;
        cluster_msg.centroid.z = ans.pose.position.z;
      }
...
private:
  ...
  ros::Subscriber dynamic_points_sub;
